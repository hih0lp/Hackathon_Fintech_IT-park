...
def get_settings() -> Settings:
    skill_dir = Path(
        os.getenv(
            "SKILL_DIR",
            os.getenv("DATA_PROTECTION_SKILL_DIR",
                      str(ROOT_DIR / "skill" / "data-protection-agent")),
        )
    )
    ...
    return Settings(
        ...
        anthropic_max_tokens=_get_env_int(
            "MAX_TOKENS",
            _get_env_int("DATA_PROTECTION_MAX_TOKENS",
                         _get_env_int("ANTHROPIC_MAX_TOKENS", 8192)),
        ),
        anthropic_temperature=_get_env_float(
            "ANTHROPIC_TEMPERATURE",
            _get_env_float("DATA_PROTECTION_TEMPERATURE", 0.1),
        ),
