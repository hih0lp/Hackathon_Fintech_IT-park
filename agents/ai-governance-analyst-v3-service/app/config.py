...
def get_settings() -> Settings:
    skill_dir = Path(
        os.getenv(
            "SKILL_DIR",
            os.getenv("AI_GOVERNANCE_SKILL_DIR",
                      str(ROOT_DIR / "skill" / "ai-governance-analyst-v3")),
        )
    )
    ...
    return Settings(
        ...
        anthropic_max_tokens=_get_env_int(
            "MAX_TOKENS",
            _get_env_int("AI_GOVERNANCE_MAX_TOKENS",
                         _get_env_int("ANTHROPIC_MAX_TOKENS", 8192)),
        ),
        anthropic_temperature=_get_env_float(
            "ANTHROPIC_TEMPERATURE",
            _get_env_float("AI_GOVERNANCE_TEMPERATURE", 0.05),
        ),
