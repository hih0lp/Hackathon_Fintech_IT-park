# Ambiguilty Resolver 0lvl Service

Легковесный Python-микросервис для снятия продуктовой и регуляторной неоднозначности с обязательным использованием скилла `ambiguilty-resolver-0lvl.skill` и потоковой интеграцией с Anthropic Claude API.

## Что делает сервис

- Загружает инструкции агента из:
  - `skill/ambiguity-resolver-v2/SKILL.md`
  - `skill/ambiguity-resolver-v2/references/regulations.md`
...
```bash
curl -N -X POST "http://localhost:8086/v1/analyze/stream" \
  -H "Content-Type: application/json" \
  -d '{
    "msg":"Добавляем P2P переводы для розницы",
    "context":{
      "region":"EU",
