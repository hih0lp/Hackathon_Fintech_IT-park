...
def get_settings() -> Settings:
    skill_dir = Path(
        os.getenv(
            "SKILL_DIR",
            os.getenv("AMBIGUITY_RESOLVER_SKILL_DIR",
                      str(ROOT_DIR / "skill" / "ambiguity-resolver-v2")),
        )
    )
    ...
    return Settings(
        ...
        anthropic_max_tokens=_get_env_int(
            "MAX_TOKENS",
            _get_env_int("AMBIGUITY_RESOLVER_MAX_TOKENS",
                         _get_env_int("ANTHROPIC_MAX_TOKENS", 8192)),
        ),
        anthropic_temperature=_get_env_float(
            "ANTHROPIC_TEMPERATURE",
            _get_env_float("AMBIGUITY_RESOLVER_TEMPERATURE", 0.0),
        ),
