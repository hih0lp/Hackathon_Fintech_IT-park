...
def get_settings() -> Settings:
    skill_dir = Path(
        os.getenv(
            "SKILL_DIR",
            os.getenv("RESULT_AGGREGATOR_SKILL_DIR",
                      str(ROOT_DIR / "skill" / "result-aggregator")),
        )
    )
    ...
    return Settings(
        ...
        anthropic_max_tokens=_get_env_int(
            "MAX_TOKENS",
            _get_env_int("RESULT_AGGREGATOR_MAX_TOKENS",
                         _get_env_int("ANTHROPIC_MAX_TOKENS", 8192)),
        ),
        anthropic_temperature=_get_env_float(
            "ANTHROPIC_TEMPERATURE",
            _get_env_float("RESULT_AGGREGATOR_TEMPERATURE", 0.0),
        ),
